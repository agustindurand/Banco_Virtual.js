...
function convertirMoneda() {
    ...
    var pesoArgentino = parseFloat(document.getElementById("pesoArgentino").value);
    // Declaracion de variable de dolarEstadounidense con su operacion correspondiente //
    var dolarEstadounidense = pesoArgentino / 448;
...
function calcularPrestamo() {
    ...
    var ingresoNeto = parseFloat(document.getElementById("ingresoNeto").value);
    ...
    if (ingresoNeto > 100000) {
        var tienePrestacion = confirm("¿Tiene alguna prestación del banco actualmente?");
        if (tienePrestacion) {
            ...
            var interes = 0.12; // Tasa de interés del 12%
            var totalIntereses = prestamo * interes * cuotas;
            var totalPrestamo = prestamo + totalIntereses;
